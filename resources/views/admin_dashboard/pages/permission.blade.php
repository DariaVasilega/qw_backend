@if($id)
    <div hx-ext="client-side-templates" _="on htmx:load remove @hx-ext">
        <div hx-get="{{ url("/permission/$id") }}" hx-trigger="load" nunjucks-template="permission_form" hx-indicator="#permission_form_spinner" _="on htmx:load remove @hx-indicator">
            <div id="permission_form_spinner" class="flex justify-center">
                <img alt="Spinner" class="h-50 w-50" src="https://media.tenor.com/On7kvXhzml4AAAAj/loading-gif.gif"/>
            </div>
            <template id="permission_form">
@endif
            <form class="flex flex-col m-auto w-full md:w-10/12 lg:w-1/2 max-w-2xl border-2 border-gray-300 border-dashed rounded-lg p-6">
                <fieldset>
                    <legend class="mb-4 flex justify-between items-center w-full">
                        ...
                        @if($disabled)
                            <span class="flex">
                                @if(in_array('permission_update', $permissions, true))
                                    <a href="#permission-id-@{{ data.permission.code }}-edit" class="mx-2 text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/permission?id=') }}@{{ data.permission.code }}" hx-target=".content">
                                        <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="m13.835 7.578-.005.007-7.137 7.137 2.139 2.138 7.143-7.142-2.14-2.14Zm-10.696 3.59 2.139 2.14 7.138-7.137.007-.005-2.141-2.141-7.143 7.143Zm1.433 4.261L2 12.852.051 18.684a1 1 0 0 0 1.265 1.264L7.147 18l-2.575-2.571Zm14.249-14.25a4.03 4.03 0 0 0-5.693 0L11.7 2.611 17.389 8.3l1.432-1.432a4.029 4.029 0 0 0 0-5.689Z"/>
                                        </svg>
                                    </a>
                                @endif
                                @if(in_array('permission_delete', $permissions, true))
                                    <a href="#permissions" class="mx-2 text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-delete="{{ url('/permission') }}/@{{ data.permission.code }}" hx-target="closest form" hx-swap="delete" _="on click wait 0.1s htmx.ajax('GET', '{{ url('/admin/page/permissions') }}', '.content')">
                        ...
                        @else
                            <div hx-ext="client-side-templates" _="on htmx:error set errors to JSON.parse(event.detail.errorInfo.xhr.response).error.description then for inputName in Object.keys(errors) add .border-red-600 to the <[id=`${inputName}`] /> then add .focus:border-red-600 to the <[id=`${inputName}`] /> then add .text-red-600 to the <[for=`${inputName}`] /> then add .peer-focus:text-red-600 to the <[for=`${inputName}`] /> then remove .hidden from <[id=`${inputName}_error`] /> then put errors[inputName].join('<br/>') into <[id=`${inputName}_error`] > span /> end">
                                <div @if($id) hx-put="{{ url("/permission/$id") }}" @else hx-post="{{ url('/permission') }}" @endif hx-include="[name]" nunjucks-template="response_message" hx-target=".content">
                    ...
                    <div class="my-4">
                        <div class="relative">
                            <input type="text" name="code" id="code" aria-describedby="code_error" onload="window.permissionCode = this.value" onchange="window.permissionCode = this.value" class="block px-2.5 pb-1.5 pt-3 w-full text-lg text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " @if($id) value="@{{ data.permission.code }}" @endif {{ $disabled }} _="on focus add .hidden to the <[id='code_error'] /> then remove .border-red-600 from me then remove .focus:border-red-600 from me then remove .peer-focus:text-red-600 from <[for='code'] /> then remove .text-red-600 from <[for='code'] />"/>
                    ...
                    <div class="my-4">
                        <div class="relative">
                            <input type="text" name="label" id="label" aria-describedby="label_error" class="block px-2.5 pb-1.5 pt-3 w-full text-lg text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " @if($id) value="@{{ data.permission.label }}" @endif {{ $disabled }} _="on focus add .hidden to the <[id='label_error'] /> then remove .border-red-600 from me then remove .focus:border-red-600 from me then remove .peer-focus:text-red-600 from <[for='label'] /> then remove .text-red-600 from <[for='label'] />"/>
...
<template id="response_message">
    <div class="flex justify-center" hx-on::load="window.location.hash=`#permission-id-${window.permissionCode}-view`; htmx.ajax('GET', `{{ url('/admin/page/permission?disabled=true&id=') }}${window.permissionCode}`, '.content')">

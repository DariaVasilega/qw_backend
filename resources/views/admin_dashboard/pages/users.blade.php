    ...
    <div hx-get="{{ url("/users?page=$page") }}" hx-trigger="load" nunjucks-template="users_list" hx-indicator="#users_list_spinner" _="on htmx:load remove @hx-indicator">
        ...
        <template id="users_list">
            {% if data %}
                        ...
                        <thead class="text-xs text-gray-700 capitalize bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                @if(in_array('user_delete', $permissions, true))
                                    <th scope="col" class="px-6 py-3"></th>
                                @endif
                                @if(in_array('user_create', $permissions, true))
                                    <th scope="col" class="px-6 py-3 text-center">
                                        <a href="#user" class="text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user') }}" hx-target=".content">
                                ...
                                <!-- {% for user in data.users %} -->
                                    <tr class="odd:bg-white odd:border-b odd:hover:bg-gray-200 odd:hover:border-gray-200 even:border-b even:bg-gray-50 even:hover:bg-gray-300 even:hover:border-gray-200">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <a href="#user-id-@{{ user.id }}-view" class="text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user?id=') }}@{{ user.id }}&disabled=true" hx-target=".content">
                                        ...
                                        @if(in_array('user_update', $permissions, true))
                                            <td class="px-6 py-4 text-center">
                                                <a href="#user-id-@{{ user.id }}-edit" class="text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user?id=') }}@{{ user.id }}" hx-target=".content">
                                                    <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        ...
                                        @if(in_array('user_delete', $permissions, true))
                                            <td class="px-6 py-4 text-center">
                                                <a href="#users-p{{ $page }}" class="text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-delete="{{ url('/user') }}/@{{ user.id }}" hx-target="closest tr" hx-swap="delete" _="on click wait 0.5s htmx.ajax('GET', '{{ url('/admin/page/users') . "?page=$page" }}', '.content')">
                ...
                <x-listing.pagination :url="url('/admin/page/users')" :view="'users'" />
            {% endif %}
        </template>
    </div>
</div>

                        ...
                        <thead class="text-xs text-gray-700 capitalize bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                @if(in_array('user_create', $permissions, true))
                                    <th scope="col" class="px-6 py-3 text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user') }}" hx-target=".content">
                            ...
                            {% if data.users %}
                                <!-- {% for user in data.users %} -->
                                    <tr class="odd:bg-white odd:border-b odd:hover:bg-gray-200 odd:hover:border-gray-200 even:border-b even:bg-gray-50 even:hover:bg-gray-300 even:hover:border-gray-200">
                                        ...
                                        <td class="px-6 py-4 text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user?id=') }}@{{ user.id }}&disabled=true" hx-target=".content">
                                        ...
                                        @if(in_array('user_update', $permissions, true))
                                            <td class="px-6 py-4 text-gray-400 transition duration-75 hover:text-gray-900 hover:cursor-pointer" hx-get="{{ url('/admin/page/user?id=') }}@{{ user.id }}" hx-target=".content">
                                                <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
